...
const SECTION_BLURB: Record<Exclude<Category, 'ignored'>, string> = {
  architecting: 'designing new systems, structure, APIs, or overall approach (including code structure and performance changes)',
  tuning: "refining the agent's output — iterating, correcting, re-prompting, adjusting tone/format/scope — plus questions and investigation",
  bugfixing: 'diagnosing and fixing defects, errors, or failing tests, plus triaging and remediating PR review feedback',
  housekeeping: 'routine upkeep — checks and release chores, docs and config edits, syncing conventions, scope-trimming, and everything else',
};
...
export function digest(opts: DigestOptions = {}): string {
  ...
  console.log(`rudder: ${rows.length} prompts on ${day}; classifying...`);
