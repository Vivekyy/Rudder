...
Pull requests to `main` run an `Enforce .agentsignore` workflow that detects agent authorship from commit authors, `Co-authored-by` trailers, and the PR author's login [@agentsignore-workflow]. The workflow always publishes an `agentsignore-policy` check run; when an agent is detected, it checks changed paths against `.agentsignore` from both the base and head revisions, then fails only when the head revision still protects a changed path [@agentsignore-workflow]. When the head rules allow a path that the base rules protected, it publishes a neutral `agentsignore-policy` check to show that the pull request explicitly relaxed protection. PRs without detected agent authorship receive a successful policy check explaining that enforcement was not required [@agentsignore-workflow]. Treat a protection relaxation as an intentional policy change, not a routine workaround; otherwise move the change to an unprotected path or ask the user for a different route.
