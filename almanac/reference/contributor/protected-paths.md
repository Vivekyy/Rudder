...
Agent-protected paths are repository paths listed in `.agentsignore` and evaluated by the pull-request enforcement workflow when a pull request has detected agent authorship. The current `.agentsignore` protects the root `README.md`, `LICENSE`, `CLAUDE.md`, the `/assets/` directory, and root agent-instruction directories [@agentsignore]. CI detects agent authorship from commit authors, `Co-authored-by` trailers, or the PR author's login; only detected-agent pull requests have their changed paths checked against `.agentsignore` rules and receive an `agentsignore-policy` check run [@agentsignore-workflow].
...
| `LICENSE` | Protects files named `LICENSE` [@agentsignore]. |
| `CLAUDE.md` | Protects files named `CLAUDE.md` [@agentsignore]. |
| `/assets/` | Protects the root `assets/` directory and paths beneath it [@agentsignore]. |
| `/.claude/` | Protects root Claude agent instructions and commands [@agentsignore]. |
| `/.codex/` | Protects root Codex agent skills and instructions [@agentsignore]. |
| `/.cursor/` | Protects root Cursor agent configuration [@agentsignore]. |
