...
The `.agentsignore` enforcement workflow first runs `detect-agent-authorship` on pull requests to `main`. It checks commit authors and `Co-authored-by` trailers between the base and head SHAs, plus the PR author's login, and exposes whether it detected an agent [@agentsignore-workflow]. The workflow always runs `enforce-agentsignore` and grants `checks: write` so that job can publish an `agentsignore-policy` check run on the PR head SHA. When no agent attribution is detected, the job publishes a successful check explaining that enforcement was not required. For detected-agent pull requests, it checks out the repository with full history, writes changed paths between the PR base and head SHAs to a temporary file, then evaluates those paths against `.agentsignore` rules from both the base and head revisions when each revision has the file [@agentsignore-workflow].
