...
- **Publishing is automatic on merge to `main`.** `.github/workflows/publish.yml`
  runs on every push to `main`: it publishes missing versions to npmjs.org via
  OIDC and to GitHub Packages via `GITHUB_TOKEN`, pushes the `v<version>` tag
  after npmjs.org is satisfied, and creates a GitHub Release with autogenerated
  notes. Just land the version bump — no manual tagging. (Forgetting to bump =
  no new npmjs.org publish.) Tests run separately in
...
- **PRs that bump the version get a release alert.**
  ...
  a version bump never lands unnoticed.
