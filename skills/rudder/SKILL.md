...
4. Identify unit tests which were added since the merge base.
   Create a recoverable backup for the paths related to these unit tests:
...
5. Turn directly expressed user intent into behavioral requirements.
   ...
   Otherwise flag it as an ambiguity, preparing for it to be deleted.
...
7. After confirmation, restore only the flagged test paths to the merge-base state.
   Never use `git reset --hard`, broad `git clean`, or change production files.
8. Identify failures and uncovered code that depends on missing intent.
   Ask one concrete question whose answer can clarify missing intent.
   Do not ask for facts inferable from already captured prompts.
9. Incorporate the answer, generate tests encoding the user's intent, and adjust code
    as needed to match user intent.
10. Rerun tests and coverage, and continue from step 8 until the configured target passes
