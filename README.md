...
| `rudder hook codex` | *(internal)* Record a Codex prompt — invoked by the hook. |
...
compilation use the same local `claude`/`codex` CLI as the digest — nothing is
sent anywhere else. To wipe everything, delete `~/.rudder/`. To stop recording,
