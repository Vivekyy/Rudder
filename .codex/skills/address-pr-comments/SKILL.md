...
6. Re-run validation. After applying any fixes, re-run the package checks — `npm run typecheck`, `npm test`, `npm run build`. Do **not** invoke the `check-changed-folders` skill from here: that skill calls this one, and re-entering it would loop. Do not commit if any check fails.

7. Report. Produce a per-comment table with these columns:
...
- Inline comment objects include `path`, `line` (or `original_line` if the line moved since the comment), `commit_id`, and `body`. Use the path and the most recent `line` value.
