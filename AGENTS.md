...
GitHub Release. It mirrors `publish.yml`'s checks — npmjs.org, GitHub Packages,
and GitHub Releases are checked directly so an existing version can be backfilled
under a renamed package. The comment is updated in place on each push (via a
hidden `<!-- release-alert -->` marker) rather than duplicated.
...
2. publish to npmjs.org if `package.json`'s name/version is missing there, using
   a **Trusted Publisher (OIDC)** — there is no `NPM_TOKEN` secret. The release
   path does **not** run tests itself — that's `test.yml`'s job (see CI above).
   `npm publish` still runs `prepublishOnly` (typecheck + test + build) as an
   intrinsic guard, so a broken tree can't ship.
3. publish to GitHub Packages via `GITHUB_TOKEN` if that registry is missing
   `@ruddercode/rudder-core@<version>`. The `repository` field in `package.json` links the
   GitHub package back to this repo so it appears under the repo's Packages.
4. push the `v<version>` tag if it is missing, **after** npmjs.org and GitHub
   Packages are satisfied (so a failed publish leaves no new tag and a re-run
   retries cleanly).
...
So the usual way to publish a new npmjs.org release is to land a version bump
(`npm version patch --no-git-tag-version`, see the PR process) on `main`. The
workflow keys off `package.json`'s name/version in each registry, so a package
rename can intentionally republish the existing version under the new name.
Forgetting to bump means no new npmjs.org publish when the current name/version
already exists (safe); the workflow can still backfill GitHub Packages or a
missing GitHub Release for an already-tagged version.

After a publishing PR merges, verify the `Publish packages and releases` workflow
succeeded, `npm view @ruddercode/rudder-core version` reflects the new release, the package
...
- The git tag is created **after package registries are satisfied**, so it is a
  marker, not the trigger. Do **not** push `v*` tags by hand — and never run
  `npm version patch` on `main` (it would bump a second time).
- If a publish ever fails before a missing tag is written (e.g. a transient npm
  error), re-run `Publish packages and releases` from the Actions tab
  (`workflow_dispatch`) and it retries the missing registry and release artifacts.
...
deleted. Prefer a **global install** (`npm i -g @ruddercode/rudder-core`) so the hook
